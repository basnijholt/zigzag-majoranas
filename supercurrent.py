...
def wrapped_current(
    ...
    current_operator = kwant.operator.Current(
        syst_wrapped, onsite=sigz, where=cut_sites
    )
    ...
    def f(k):
        ...
        I = sum(
            fermi_dirac(e.real, p) * local_current_operator(ev)
    ...
    runner = adaptive.runner.simple(
        learner,
        lambda l: l.done()
        or l.npoints > max_points
        or (
            l.npoints > 20
            and np.max(np.abs(list(learner.done_points.values()))) < zero_current
        ),
    )
