...
The combination of these ensures a significant decrease of the Majorana decay length because $\xi_M \propto v_\textrm{F}/E_\textrm{gap}$
...
To determine $\xi_\textrm{M}$, we use a trick and artificially break the particle-hole symmetry on one edge by adding a potential $V \sigma_0 \tau_0$ such that one of the Majorana states is pushed into the continuum, then the fit to $\exp{x / \xi_\textrm{M}}$ becomes more robust.
