...
To optimize the sampling in important regions of parameter space, we use the Adaptive package.~\cite{adaptive}
...
We diagonalize the supercell Hamiltonian to compute the band structure.
...
The introduction of a zigzag has a striking effect: the bands flatten out and the topological gap increases by more than an order of magnitude.
...
Besides this, the states that exist in different segments have very little overlap and therefore have a very small velocity.
This reduction in velocity has a positive effect on the Majorana coherence length, as described later in section \ref{sec:shape_effects}.
...
We compute the dispersion of the translationally invariant realizations of a straight and saw-toothed zigzag system and find $\min \left| {E(k)} \right|$ to obtain the energy gap.  % TODO: mention this in main text!
Additionally, in subplot (a) we plot the phase boundaries because they are too narrow to see.
We do this by solving a generalized eigenvalue problem to find all phase boundaries at once as described in Ref.~\onlinecite{nijholt2015orbital}.  % TODO: mention in main text
% TODO: mention distinguish between min E(K) and other algo in main text!
