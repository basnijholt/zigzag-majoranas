...
Inverting $\alpha\left(E\right)\equiv\exp\left(-i\arccos\left(E/\Delta\right)\right)$ gives $\frac{E}{\Delta}=\frac{\alpha^{2}+1}{2\alpha}=\frac{1}{2}\left(\alpha+\alpha^{-1}\right)$, and since $\alpha$ is unitary and only defined in the postitive imagionary plane, $E_{i}=\textrm{real}\left(\pm\sqrt{1/\lambda_{i}}\right)$.  % TODO: add the mathematical notation for taking the lambda.imag > 0.
