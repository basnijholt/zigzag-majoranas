...
An important challenge in creating stable Majoranas is the appearance of a soft gap---a power law decay instead of an exponential decay of the density of states near zero energy.~\cite{liu_soft_gap_phenomenology}
It has been shown that interface disorder, pairbreaking, tunnel barrier to normal leads, temperature, and having multimodes with different transmission probabilities, and dissipation can cause a soft induced gap~\cite{takei_soft_2013,liu_soft_gap_phenomenology,stanescu_soft_gap_2014}.
However, even without the aforementioned causes, a soft gap would still appear due to states with their momentum parallel to the junction~\cite{de_gennes_1963,beenakker_billiards_2005}.
