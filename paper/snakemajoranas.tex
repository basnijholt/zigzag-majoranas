...
In figure~\ref{fig:wave_functions} the Majorana wave function densities $\left| \psi_\textrm{M} \right|^2$ are displayed, superimposed upon their respective geometry.
...
Altogether, the small topological gap, but also the before mentioned high velocity, result in poorly localized Majoranas and minimal topological protection against perturbations because

\begin{equation}
\label{eq:xi_M}
\xi_\textrm{M}=\hbar \frac{v_\textrm{F}}{\Delta_\textrm{ind}}.
\end{equation}
...
Further, the topological gap $E_\textrm{gap}$ is an order of magnitude higher, and as mentioned in section \ref{sec:band_structures}, the velocity $v_\textrm{F}$ is an order of magnitude lower.
Both of these parameters ensure that $\xi_\textrm{M}$ is greatly improved (Eq.~\eqref{eq:xi_M}).
