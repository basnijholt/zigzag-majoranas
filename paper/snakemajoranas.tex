...
\date{\today}
\begin{abstract}
Quasi-particle trajectories in high density superconductor-semiconductor-superconductor junctions that have a large momentum parallel to the junction have a very long flight time, and therefore a small induced superconducting gap.
...
We show that a zigzag-shaped geometry eliminates these trajectories, at the same time allowing the robust creation of Majorana states with both the gap and the Majorana size improved by more than an order of magnitude for realistic parameters.
...
Additionally, these long trajectories have long flight times $\tau_f=v_F(\theta) / D$ (see Fig.~\ref{fig:setup}(a)) and equivalenly small Thouless energies $E_{\textrm{Th}}=\hbar / \tau_f$, resulting in a small gap. % TODO: fix the Eq for tau_f
...
To determine $\xi_\textrm{M}$, we artificially break the particle-hole symmetry on one edge by adding a potential $V \sigma_0 \tau_0$ such that one of the Majorana states is pushed into the continuum and the fit to $\exp{\frac{x}{\xi_\textrm{M}}}$ becomes easier.
...
We do this by solving a generalized eigenvalue problem to find all phase boundaries at once as described in Ref.~\onlinecite{nijholt2015orbital}.
