...
Besides this, the velocity of the modes greatly reduces, which has a positive effect on the Majorana coherence length, as described later in section XXX. % TODO: add ref
...
Because the introduction of the zigzag shape moves the system to class D, we see a cleaner topological region due to the absence of the gap closings resulting from the BDI symmetry.
...
We observe that once there are no more straight trajectories possible inside the junction (when $z_y=\frac{W}{2}$) the spectrum becomes most insensitive to the momentum $k_x$.
...
Further, the energy of the Majorana state in the straight system is badly septated from the next lowest lying eigenstate.
Altogether, for the parameters of the system, the Majoranas are very poorly localized and topological protection against perturbations is minimal.

\comment{In a zigzag geometry Majoranas are localized within one segment of zigzag.}
All of the zigzag type geometries show a reduced coherence length.
