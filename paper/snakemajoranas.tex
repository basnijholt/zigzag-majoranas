...
\comment{An alternative has been proposed.}
...
We propose a new experimental setup (see Fig.~\ref{fig:setup}(b)) for the creation of Majoranas that eliminating long trajectories and therefore prevents the appearance of a soft gap, while also increasing the topological gap by several orders of magnitude, depending on the parameters.
...
In figure~\ref{fig:band_structures} we show the resulting band structures for zigzag systems with varying amplitude.
...
With (a) a straight system, (b) a saw-toothed zigzag system, (c) a system where lines parallel to a sinusoid defines the normal region, and (d) similar to (c) but with disordered edges.
...
To determine $\xi_\textrm{M}$ for the zigzag systems, we use a trick and artificially break the particle-hole symmetry on one edge by adding a potential $V \sigma_0 \tau_0$ such that one of the Majorana states is pushed into the continuum.
Using this system, we then find the Majorana size $\xi_\textrm{M}$ by fitting an exponential to the density of the single Majorana wave function projected on the $x$-axis.
This method is numerically not feasible for a straight system because the decay length is too long.
Instead, we find $\xi_\textrm{M}$ through the eigenvalue decomposition of the translation operator at zero energy as described in Ref.~\onlinecite{nijholt2015orbital}.
All systems have identical Hamiltonian parameters to compare the effects of the changing geometry meaningfully.

\comment{In a straight system, the Majoranas are very poorly localized.}
For the straight system [Fig.~\ref{fig:wave_functions}(a)], we see that the decay of the density is long compared to the system size.
...
Altogether, the small topological gap, but also the before mentioned high velocity, result in poorly localized Majoranas and minimal topological protection against perturbations.  % TODO: probably put the eq for coherence length here?

\comment{In a zigzag geometry Majoranas are localized within one segment of zigzag independent of details}
We observe that for zigzag systems the Majorana properties improve independent of the details of the specific geometry.
All of the zigzag type geometries display a significantly reduced Majorana size which improves the localization of the Majorana wave functions as is visible in the density plots.
The Majorana size $\xi_\textrm{M}$ improves by a factor ~70 and is localized within one segment of the zigzag.
Further, the topological gap is an order of magnitude lower, and as mentioned in section \ref{sec:band_structures}, this can be attributed to the way the energy gap and velocity factor into the Majorana coherence length which scales with these parameters $\xi_\textrm{M}=\hbar v_\textrm{F} / \Delta_\textrm{ind}$.
Additionally, the reduction of the Majorana size allows for shorter systems sizes, which is favorable because in an experimental device the coherence length of a material sets a limit for the device size.
