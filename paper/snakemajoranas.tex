...
Later in this manuscript, we show that the exact details of the shape are not of importance and consider different zigzag geometries also parametrized by $z_x, \; z_y, \; W$.
...
Unless noted differently, the parameters of the system are $\alpha=\SI{20}{\meV \nm}$, $g=26$, $\meff=\SI{0.02}{\electronmass}$, $\mu=\SI{20}{\meV}$, $\Delta=\SI{1}{\meV}$.
...
The values of the remaining parameters are $\alpha=\SI{20}{\meV \nm}$, $g=26$, $\meff=\SI{0.02}{\electronmass}$, $\mu=\SI{20}{\meV}$ , $\Delta=\SI{1}{\meV}$ , $B=\SI{1}{\tesla}$, $\phi=2\pi$.\label{fig:wavefunctions}}
\end{figure}

\comment{We calculate the wave functions and find the Majorana lengths by fitting an exponential.}
We model a finite system to compute the Majorana wave function density for different geometries; ribbon, zigzag, parallel curve, and variants of the former with disordered edges.
...
Altogether, the increase in the topological gap, but also the before mentioned reduction of the group velocity, result in poorly localized Majoranas and minimal topological protection against perturbations.  % XXX: probably put the eq for coherence length here?
...
Current fabrication techniques are compatible with the proposed geometry; zigzag devices have already been fabricated. % XXX: add ref to Fokko's thesis
