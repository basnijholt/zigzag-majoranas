...
From a semiclassical perspective, long paths through the semiconductor without interruption by the superconductor.

\comment{Currently proposed workarounds are low density or disorder, but both have drawbacks.}
We claim that one of the problems lying in the way of a stable system, is the appearance of a soft gap; the gap is greatly reduced for states with momentum directed along the length of the strip.

\comment{We show that zigzag geometry solves this problem by eliminating the long trajectories.}
We show that introducing a zigzag or snake like geometry for the semiconductor removes these long trajectories and avoids the appearance of a soft gap.
...
We consider a system (Fig.~\ref{fig:setup}) consisting of a two dimensional strip of semiconductor, with superconductors adjacent on both sides.
As seen in the figure, we modulate the shape of the normal region, which can be either zigzag as depicted, or a more smooth snake like shape.
Just as the normal system[\cite{pientka2017topological}], a magnetic field pointed along the x direction is applied.
We model the system with a BdG Hamiltonian Eq.~\eqref{eq:hamiltonian}; the normal part has Rashba spin-orbit coupling and a Zeeman field, and apart from the kinetic term, the superconductors only have a superconducting term differ in superconducting phase.
...
We consider the model described in section II; a two dimensional SNS junction with a Rashba BdG Hamiltonian.
...
In order to create a topological phase diagram we would normally compute the topological invariant(pfaffian).
For a ribbon this is possible, but computing the invariant of the supercell is too computationally expensive.
Using a generalized eigenvalue problem also proved unsuccessful due to numerical errors, so we resort to simply calculating the gap and noting the gap closings.
