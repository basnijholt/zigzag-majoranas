...
In the unfolded band structure for a straight system, as seen in Fig.~\ref{fig:bandstructures}(a), the gap in a straight system occurs around $k_F$ and because the zigzag cuts off long trajectories, the gap size increases.
...
Additionally, we plot the first $\SI{1300}{\nm}$ (one zigzag period) of the wavefunction at the most optimal point in parameter space for the straight system.
For the straight system we calculate $E_\textrm{gap}$ by performing a binary search in $E$ for the energy at which the propagating modes start to appear.
