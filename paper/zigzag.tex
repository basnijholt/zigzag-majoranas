...
Besides short trajectories having a larger $E_\textrm{gap}$, we also find that they are less sensitive to the alignment of the magnetic field.

\comment{Zigzag geometry removes the need for precise field alignment.}
Because the zigzag geometry cuts off long trajectories---leaving only short trajectories---this device will require a less precise alignment of the magnetic field.
To show this, we have performed a simulation of a straight and zigzag device where we rotate the field, and the results are in Fig.~\ref{fig:tilted_angle_phase_diagram}.
We observe that at a $10^\circ$ misaligned magnetic field, the zigzag device still has a sizeable gap, wherein the staight device a $1^\circ$ misalignment would be detrimental.
Additionally, we simulate a device where the magnetic field is parallel to the NS interface and compare it with a field purely in the $x$-direction, the results are in Fig.~\ref{fig:snaking_magnetic_field}.
From these results we observe that the gap enhancement effect persists.
...
On the x-axis is the magnetic field $B$ and on the $y$-axis is the angle $\theta=\arctan{B_y/B_x}$ in degrees.
...
\begin{figure}[!htb]
\centering
\includegraphics[width=0.9\columnwidth]{figures/snaking_magnetic_field.pdf}
\caption{
Band structures where the magnetic field is parallel to the NS interface (orange) and a magnetic field in the $x$-direction (blue) for different values of $z_y$.
...
To verify that the change in the interface transparency alone is not sufficient to explain the increase of the topological gap, we have simulated the dispersion of a system with $z_x = z_y = \SI{80}{\nm} \approx v_\textrm{F}$ and $W=\SI{400}{\nm}$, so that the angle of the boundaries is sufficiently large, but the boundary modulation is much smaller than the width.
We have observed that $E_\textrm{gap}$ became $\SI{6.1}{\mu \eV}$, a 2x increase compared to the straight system.
