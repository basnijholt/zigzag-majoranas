...
These long trajectories have long flight times $\tau_\textrm{f} \approx L_\textrm{t} / v_\textrm{F}$ (see Fig.~\ref{fig:setup}), where $L_\textrm{t}$ is the trajectory length and $v_\textrm{F}$ the Fermi velocity.
...
The zigzag geometry increases the topological gap in the high density regime by more than an order of magnitude, as well as substantially reduces Majorana size.
...
Finally, we have neglected several important physical effects, such as: disorder, electrostatics, the orbital effect of magnetic field, and the finite thickness of the sample.
...
The zigzag geometry both increases the interface transparency (as particles typically hit the superconductoer
closer to normal incidence) as well as sets an upper limit to the trajectory length.
Here we show that the change in the interface transparency alone is not sufficient to explain the increase of the topological gap.
