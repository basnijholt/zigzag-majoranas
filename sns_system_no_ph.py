...
def make_ns_junction(
    ...
    **_
    ...
    normal_lead = kwant.Builder(
        normal_lead_symmetry, conservation_law=conservation_matrix
    )
    ...
    barrier = kwant.Builder(
        symmetry=wraparound_symmetry, conservation_law=conservation_matrix
    )
    ...
    normal_lead = kwant.wraparound.wraparound(
        normal_lead, keep=1
    )  # Keep lead in y-direction
    sc_lead = kwant.wraparound.wraparound(sc_lead, keep=1)  # Keep lead in y-direction
    ...
    **_
    ...
    def within_zigzag(z_x, z_y, offset):
        def shape(pos):
            x, y = pos.pos
            return (
                below_zigzag(z_x, z_y, (offset[0], offset[1] + L_m))(pos)
                and above_zigzag(z_x, z_y, offset)(pos)
                and 0 <= x < L_x
            )
    ...
    def edge_zigzag(z_x, z_y, offset):
        def shape(pos):
            ...
            return (
                below_zigzag(z_x, z_y, (offset[0], offset[1] + L_m))(pos)
                and above_zigzag(
                    z_x, z_y, (offset[0], offset[1] + L_m - a * edge_thickness)
                )(pos)
                and 0 <= x < L_x
            ) or (
                below_zigzag(z_x, z_y, (offset[0], offset[1] + edge_thickness * a))(pos)
                and above_zigzag(z_x, z_y, (offset[0], offset[1]))(pos)
                and 0 <= x < L_x
            )
    ...
    within_zigzag_shapes = [
        within_zigzag(z_x, z_y, (2 * z_x * i, 0)) for i in range(number_of_zigzags)
    ]
    edge_zigzag_shapes = [
        edge_zigzag(z_x, z_y, (2 * z_x * i, 0)) for i in range(number_of_zigzags)
    ]
    ...
    top_shape_with_some_down = intersection_shape(
    ...
    down_shape_with_some_top = intersection_shape(
