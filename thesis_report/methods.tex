...
\section{Quasiclassical estimation of the topological gap}
	...
	\subsection{Analytical estimate of Andreev spectrum for a straight junction}
		...
		We derive the spectrum by calculating the scattering matrix $S(\epsilon)$ for the normal region and applying the Andreev bound state condition (Eq.~\eqref{eq:bound_state_condition_sjl}) for the short junction limit \cite{beenakker_universal_1991,sticlet_robustness_2017}.
		The short junction limit corresponds to the case where we can neglect the energy dependence of the scattering matrix: $S(\epsilon) \approx S(0)$.
		This limit is valid for $W<\xi$, where $\xi$ is the superconducting coherence length.
		...
		\subsubsection{Derivation of the scattering matrix}
			...
			The scattering matrix, displayed in block matrix format:
			\begin{align}
			\label{eq:smatrix}
		    S &= \left(
		    \begin{array}{rr}
