...
dummy_params = dict(**constants, **dummy_params_raw)
...
def create_parallel_sine(distance, z_x, z_y, rough_edge=None):
    ...
    if rough_edge is not None:
        ...
        rands = [
            ...
            *rands,
            0,
        ]  # make sure that it starts and ends with 0 (for periodicity)
        ys_disorder = scipy.interpolate.interp1d(
            np.linspace(0, z_x, len(rands)), rands, kind="quadratic"
        )(xs)
...
def get_templates(
    ...
    return (template_barrier, template_normal, template_sc_left, template_sc_right)
...
def make_system(
    ...
    if sc_leads and not infinite or sc_leads and not wraparound:
        raise ValueError(
            "If you want to use sc_leads, infinite and wraparound must be True."
        )
    ...
    if parallel_curve:
        ...
        curve_top = create_parallel_sine(
            L_m // 2, z_x, z_y, rough_edge=(X, Y, salt) if rough_edge else None
        )
        below_shape = below_curve(curve_top)

        curve_bottom = create_parallel_sine(
            -L_m // 2, z_x, z_y, rough_edge=(X, Y, -salt) if rough_edge else None
        )
        ...
        _curve = (
            lambda x: 4 * z_y / z_x * (x % (z_x / 2)) - z_y
            if x % z_x < z_x / 2
            else -4 * z_y / z_x * (x % (z_x / 2)) + z_y
        )
    ...
    if sawtooth:
        ...
        top_superconductor_shape = middle_shape.inverse()[
            0:L_x, : L_sc_up + y_offset // 2 + z_y
        ]
    else:
        top_superconductor_initial_site = (z_x // 4, L_m // 2 + z_y + L_sc_up // 2)
        top_superconductor_shape = middle_shape.inverse()[
            0:L_x, : L_sc_up + L_m // 2 + z_y
        ]
    ...
    if sawtooth:
        ...
        bottom_superconductor_shape = middle_shape.inverse()[
            0:L_x, -L_sc_down - y_offset // 2 - z_y :
        ]
    else:
        bottom_superconductor_initial_site = (
            z_x // 4,
            -L_m // 2 - z_y - L_sc_down // 2,
        )
        bottom_superconductor_shape = middle_shape.inverse()[
            0:L_x, -L_sc_down - L_m // 2 - z_y :
        ]
    ...
    edge_shape = (
        middle_shape.edge()
        * bottom_superconductor_shape.outer_edge()
        * top_superconductor_shape.outer_edge()
    )
    ...
    if ns_junction:
        ...
        barrier_syst = kwant.Builder(
            kwant.TranslationalSymmetry([L_x, 0]), conservation_law=conservation_matrix
        )
        top_superconductor_lead = kwant.Builder(
            kwant.TranslationalSymmetry((L_x, 0), (0, a))
        )
        bottom_normal_lead = kwant.Builder(
            kwant.TranslationalSymmetry((L_x, 0), (0, -a)),
        ...
        top_superconductor_lead.fill(
            template_top_superconductor, top_superconductor_lead_shape, (0, 0)
        )

        bottom_normal_lead.fill(template_normal, bottom_normal_lead_shape, (0, 0))

        barrier_syst = kwant.wraparound.wraparound(barrier_syst)
        top_superconductor_lead = kwant.wraparound.wraparound(
            top_superconductor_lead, keep=1
        )
    ...
    ##################
        ...
        # --------------
        # Fill interior
        interior_sites = syst.fill(
            template_interior, interior_shape, interior_initial_site
        )
        ...
        if L_sc_up is not 0:
            top_superconductor_sites = syst.fill(
                template_top_superconductor,
                top_superconductor_shape,
                top_superconductor_initial_site,
            )
            add_to_site_colors(
                site_colors, top_superconductor_sites, "top_superconductor"
            )

        # ---------------------------
        # Fill bottom superconductor
        if L_sc_down is not 0:
            bottom_superconductor_sites = syst.fill(
                template_bottom_superconductor,
                bottom_superconductor_shape,
                bottom_superconductor_initial_site,
            )
            add_to_site_colors(
                site_colors, bottom_superconductor_sites, "bottom_superconductor"
            )
